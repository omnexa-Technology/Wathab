...
export function ServicesGridCard({
  ...
  return (
    <div
      className={`flex flex-col gap-6 px-6 py-8 rounded-3xl ${cardBg} transition-[transform,box-shadow] duration-300 ease-out hover:-translate-y-1 hover:shadow-lg motion-reduce:translate-y-0 motion-reduce:shadow-none ${className}`}
      dir={isRTL ? 'rtl' : 'ltr'}
      {...props}
    >
      {/* Icon */}
      <div className={`flex items-center justify-center w-14 h-14 shrink-0 ${iconColor}`} aria-hidden>
